...
const ModelIcon: React.FC<{ icon: string }> = ({ icon }) => {
  if (icon === "tree")     return <GitBranch size={18} className="text-blue-600 dark:text-blue-400" />;
  if (icon === "xg")       return <span className="text-xs font-black text-blue-600 dark:text-blue-400">XG</span>;
  if (icon === "zap")      return <Zap size={18} className="text-blue-600 dark:text-blue-400" />;
  if (icon === "sparkles") return <Sparkles size={18} className="text-blue-600 dark:text-blue-400" />;
    ...
    <div className="rounded-2xl border-2 border-gray-800 dark:border-gray-700 p-1 shadow-lg shadow-gray-900/20 transition-all duration-300 hover:shadow-gray-800/40 hover:border-gray-700 animate-fadeIn">
      <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 h-full">
        ...
        <div className="flex items-center gap-3 mb-4">
          <span className="flex items-center justify-center w-7 h-7 rounded-full bg-gray-900 dark:bg-gray-700 text-white text-xs font-bold flex-shrink-0 ring-2 ring-gray-700 shadow-md">2</span>
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-gray-100 leading-tight">Sélection du Modèle</h3>
            <p className="text-xs text-gray-400 dark:text-gray-500">Choisissez votre algorithme</p>
          </div>
        </div>
        ...
        {mode === "auto" && (
          <p className="text-xs text-gray-500 dark:text-gray-400 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-lg px-3 py-2 mb-3 animate-fadeIn">
            ...
            return (
              ...
              >
                <div className={`
                  ...
                  ${isSelected && !isDisabled ? "bg-white/10" : "bg-blue-50 dark:bg-blue-900/30 group-hover:bg-blue-100 dark:group-hover:bg-blue-800/50"}
                `}>
                  {isSelected && !isDisabled
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-xs font-semibold truncate transition-colors duration-200
                    ${isSelected && !isDisabled ? "text-white" : "text-gray-800 dark:text-gray-200"}`}>
                    {m.name}
                  </p>
                  <p className={`text-xs truncate transition-colors duration-200
                    ${isSelected && !isDisabled ? "text-gray-300 dark:text-gray-400" : "text-gray-400 dark:text-gray-500"}`}>
                    {m.description}
                  </p>
                </div>
