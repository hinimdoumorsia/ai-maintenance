    ...
    <div className="rounded-2xl border-2 border-gray-800 dark:border-gray-700 p-1 shadow-lg shadow-gray-900/20 transition-all duration-300 hover:shadow-gray-800/40 hover:border-gray-700 animate-fadeIn">
      <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="flex-1 text-sm font-semibold text-gray-900 dark:text-gray-100">Journaux de l'Agent</h3>
        ...
        <div className="max-h-96 overflow-y-auto space-y-1">
          {logs.length === 0 && !isRunning && (
            <div className="flex flex-col items-center justify-center py-8 gap-2 text-gray-400 dark:text-gray-500 text-xs text-center">
              <div className="w-10 h-10 rounded-xl bg-gray-50 dark:bg-gray-700 border-2 border-dashed border-gray-200 dark:border-gray-600 flex items-center justify-center">
          ...
          {logs.map((log, i) => (
            <div
              key={i}
              className="flex items-start gap-2 px-2.5 py-2 rounded-lg bg-gray-50 dark:bg-gray-700/50 border border-gray-100 dark:border-gray-700 hover:border-gray-200 dark:hover:border-gray-600 hover:bg-gray-100/60 dark:hover:bg-gray-700 transition-colors duration-150"
            >
              <span className="flex-shrink-0 mt-0.5">
                {IconMap[log.type] ?? <Cpu size={13} color="#F97316" />}
              </span>
              <span className="flex-shrink-0 text-xs text-gray-400 dark:text-gray-500 font-mono mt-0.5 whitespace-nowrap">
                {log.time}
              </span>
              <div className="flex-1 flex flex-col gap-0.5 min-w-0">
                <span className="text-xs font-semibold text-orange-500">{log.title}</span>
                {log.detail && (
                  <span className="text-xs text-gray-500 dark:text-gray-400 break-words leading-relaxed">{log.detail}</span>
                )}
              </div>
            </div>
          ))}
        ...
        {logs.length > 0 && (
          <div className="mt-2 pt-2 border-t border-gray-100 dark:border-gray-700 text-xs text-gray-400 dark:text-gray-500 text-right">
